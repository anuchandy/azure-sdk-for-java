...
public final class Tracing {
    ...
    public Tracing(String endpoint, ClientOptions clientOptions) {
        this.logger = new ClientLogger(Tracing.class);
    ...
    public ChatCompletions traceComplete(ChatCompletionsOptions request,
        BiFunction<BinaryData, RequestOptions, ChatCompletions> operation, BinaryData opArg0, RequestOptions opArg1,
        Context parent) {
        if (!tracer.isEnabled()) {
            return operation.apply(opArg0, opArg1.setContext(parent));
        }
        ...
        try (AutoCloseable ignored = tracer.makeSpanCurrent(span)) {
            final ChatCompletions response = operation.apply(opArg0, opArg1.setContext(span));
